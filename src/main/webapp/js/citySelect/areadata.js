...
strVarCity += "                      <button class=\"aui-btn aui-btn-primary\" onclick=\"saveCity()\" type=\"button\">确定<\/button>";
    ...
    if ($(dataCityinput).data("value") != "") {
        ...
        if (inputarry.length > 0) {
            for (var i in inputarry) {
                $('.data-result').append("<span class=\"save_box aui-titlespan\" data-code=\"" + inputarry[i] + "\" data-name=\"" + inputarryname[i] + "\" onclick=\"removespan_area(this)\">" + inputarryname[i] + "<i>×<\/i><\/span>");
            }
        }
    }
    ...
    //显示下一级
        ...
        if (typeof (relations[$(con).data("code")]) != "undefined") {
            ...
            if (datatype == "duoxuan") {
                ...
                if ($('.data-result span').length > 9) {
                    $('.data-error').slideDown();
                    setTimeout("$('.data-error').hide()", 1000);
                    return false;
                } else {
                    $('.data-result').append("<span class=\"save_box aui-titlespan\" data-code=\"" + $(con).data("code") + "\" data-name=\"" + $(con).data("name") + "\" onclick=\"removespan_area(this)\">" + $(con).data("name") + "<i>×<\/i><\/span>");
